...
export const POST: RequestHandler = async ({ request, url }) => {
	try {
		...
		if (reservationStart < openHour || reservationStart >= closeHour) {
			return json(
				{ error: `Rezervācijas iespējamas tikai darba laikā (${start}-${end})` },
				{ status: 400 }
			);
		}
		...
		if (reservationStart + duration > closeHour) {
			return json(
				{ error: `Rezervācijas laiks pārsniedz darba laiku (${start}-${end})` },
				{ status: 400 }
			);
		}
		...
		for (const timeSlot of consecutiveSlots) {
			...
			if (hasConflict) {
				return json(
					{ error: `Selected zones are already booked for ${timeSlot}.` },
					{ status: 409 }
				);
			}
		}
